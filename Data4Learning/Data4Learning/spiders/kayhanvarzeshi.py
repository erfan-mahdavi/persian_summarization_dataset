...
class KayhanvarzeshiSpider(BaseSpider):
    name = "kayhanvarzeshi"
    allowed_domains = ["kayhanvarzeshi.ir"]
    start_urls = ["https://kayhanvarzeshi.ir/fa/archive"]



    def parse_main(self, response):
        pages = response.css('div.archive_content div.linear_news a.title5::attr(href)').getall()
        for href in pages:
            url = "https://kayhanvarzeshi.ir" + href
        ...
        if navs:
            next = navs
            url = "https://kayhanvarzeshi.ir" + next
            if self.allowed_domains[0] in url:
                yield scrapy.Request(
                    url,
    ...
    def parse_article(self, response):
        ...
        tags = self.clean_tags(response.css('div.tags_title a::text').getall())

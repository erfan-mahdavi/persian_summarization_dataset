...
class GamefaTechSpider(BaseSpider):
    name = "gamefa_tech"
    allowed_domains = ["gamefa.com"]
    start_urls = ["https://gamefa.com/category/computer/page/2/"]

    def __init__(self, *args, **kwargs):
        super(GamefaTechSpider, self).__init__(*args, **kwargs)
        self.counter = 2
    ...
    def parse_main(self, response):
        ...
        self.counter += 1
        next_page_url = f"https://gamefa.com/category/computer/page/{self.counter}/"
    ...
    def parse_article(self, response):
        ...
        real = response.css("ul.wp-block-list li a::attr(href)").get()
        if not real:
            ...
            if self.allowed_domains[0] in real:
                yield scrapy.Request(
                    real,
    ...
    def parse_real(self, response):
        ...
        tags = self.clean_tags(response.css('div.post-tags a::text').getall())
        article = self.clean_nodes(response.css('div.post-content p ::text').getall())
        real = response.css("wp-block-list li a::attr(href)").get()
